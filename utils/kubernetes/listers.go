...
// NotReadyNodeLister returns the ReadyNodeLister registered to this registry
...
// NewNotReadyNodeLister builds a node lister that returns only not ready nodes.
func NewNotReadyNodeLister(kubeClient client.Interface, stopChannel <-chan struct{}) NodeLister {
	return NewNodeLister(kubeClient, IsNotNodeReadyAndSchedulable, stopChannel)
}
