...
type ScaleUpStatus struct {
	...
	ScaleUpError            *errors.AutorepairError
...
type ScaleUpStatusProcessor interface {
	Process(context *context.AutorepairingContext, status *ScaleUpStatus, kubeclient kube_client.Interface)
	CleanUp()
}
...
func (p *NoOpScaleUpStatusProcessor) Process(context *context.AutorepairingContext, status *ScaleUpStatus) {
}
